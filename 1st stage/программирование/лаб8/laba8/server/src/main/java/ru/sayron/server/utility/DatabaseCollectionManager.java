...
public class DatabaseCollectionManager {
    ...
    public void updateOrganizationById(long organizationId, OrganizationRaw organizationRaw) throws DatabaseHandlingException {
        ...
        PreparedStatement preparedUpdateLocationStatement = null;
        try {
            databaseHandler.setCommitMode();
            databaseHandler.setSavepoint();
            ...
            preparedUpdateLocationStatement = databaseHandler.getPreparedStatement(UPDATE_LOCATION_BY_ID, false);
            ...
            if (organizationRaw.getOfficialAddress().getTown() != null) {
                preparedUpdateLocationStatement.setFloat(1, organizationRaw.getOfficialAddress().getTown().getX());
                preparedUpdateLocationStatement.setFloat(2, organizationRaw.getOfficialAddress().getTown().getY());
                preparedUpdateLocationStatement.setFloat(3, organizationRaw.getOfficialAddress().getTown().getZ());
                preparedUpdateLocationStatement.setLong(3, getAddressIdByOrganizationId(organizationId));
                if (preparedUpdateLocationStatement.executeUpdate() == 0) throw new SQLException();
                Main.logger.info("Выполнен запрос UPDATE_LOCATION_BY_ID.");
            }
            ...
            databaseHandler.closePreparedStatement(preparedUpdateLocationStatement);
            databaseHandler.setNormalMode();
        }
    }
